...
총 데이터 세트는 k 세트 로 분할됩니다. 하나씩 (외부) 테스트 세트로 세트가 선택되고 k  - 1개의 다른 세트가 해당 외부 훈련 세트로 결합됩니다.

이것은 k 세트 각각에 대해 반복됩니다. 각 외부 훈련 세트는 l 세트 로 다시 세분화됩니다 . 

하나씩 하나의 세트가 내부 테스트(검증) 세트로 선택되고 l  - 1개의 다른 세트가 해당 내부 훈련 세트로 결합됩니다. 이것은 각 l 에 대해 반복됩니다. 
...
k-i 겹 교차 검증은 많은 양의 다양한 하이퍼 파라미터에 대해 반복되고

교차 검증 세트 중 최상의 세트가 사용된다.
